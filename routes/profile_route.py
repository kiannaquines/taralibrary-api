...
def add_profile(profile_create: ProfileCreate, db: Session = Depends(get_db)):
    try:
...
def edit_profile(profile_id: int, profile_update: ProfileCreate, db: Session = Depends(get_db)):
