...
def create_zone_image(zone_image: ZoneImageCreate, db: Session = Depends(get_db)):
...
def read_zone_images(zone_id: int, db: Session = Depends(get_db)):
    return get_zone_images(db=db, zone_id=zone_id)

@zone_img_router.delete("/zone-images/{zone_image_id}", response_model=ZoneImage)
def delete_zone_image(zone_image_id: int, db: Session = Depends(get_db)):
